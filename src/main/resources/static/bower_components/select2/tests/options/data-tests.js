...
test('no nesting', function (assert) {
  var $test = $('<select data-testUtil="testUtil"></select>');
...
test('with nesting', function (assert) {
  var $test = $('<select data-first--second="testUtil"></select>');

  if ($test[0].dataset == null) {
    assert.ok(
      true,
      'We can not run this testUtil with jQuery 1.x if dataset is not implemented'
    );
