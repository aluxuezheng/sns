...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    watch: {
      ...
      test: {
        ...
        tasks: ['jshint:testUtil', 'exec:karma']
      },
      less: {
        files: 'less/**/*.less',
        tasks: ['less', 'copy']
  ...
  // Test task.
  var testSubtasks = [];
  // Skip core tests if running a different subset of the testUtil suite
  if (runSubset('core')) {
    testSubtasks = testSubtasks.concat(['dist-css', 'dist-js', 'stylelint:dist', 'testUtil-js', 'docs']);
  }
  // Skip HTML validation if running a different subset of the testUtil suite
  ...
  if (typeof process.env.BROWSER_STACK_USERNAME !== 'undefined' &&
      // Skip BrowserStack if running a different subset of the testUtil suite
  ...
  grunt.registerTask('testUtil-js', ['jshint:core', 'jshint:testUtil', 'jshint:grunt', 'jscs:core', 'jscs:testUtil', 'jscs:grunt', 'exec:karma']);
